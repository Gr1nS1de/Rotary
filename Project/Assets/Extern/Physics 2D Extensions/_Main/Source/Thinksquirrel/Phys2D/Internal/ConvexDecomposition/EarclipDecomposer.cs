...
namespace Thinksquirrel.Phys2D.Internal.ConvexDecomposition
{
    /// <summary>
    ...
    /// </summary>
    static class EarclipDecomposer
    {
        //box2D rev 32 - for details, see http://www.box2d.org/forum/viewtopic.php?f=4&t=83&start=50

        private const float Tol = .001f;

        public static List<List<Vector2>> ConvexPartition(Vector2[] vertices)
        {
            return ConvexPartition(new List<Vector2>(vertices), int.MaxValue, 0);
        }
        ...
        ///
        /// Each resulting polygon will have no more than Settings.MaxPolygonList<Vector2>
        /// vertices.
        /// 
        ...
        /// <returns></returns>
        public static List<List<Vector2>> ConvexPartition(List<Vector2> vertices, int maxPolys, float tolerance)
        {
            if (vertices.Count < 3)
                return new List<List<Vector2>> { vertices };
            /*
            if (vertices.IsConvex() && vertices.Count <= Settings.MaxPolygonList<Vector2>)
            {
                if (vertices.IsCounterClockWise())
                {
                    List<Vector2> tempP = new List<Vector2>(vertices);
                    tempP.Reverse();
                    tempP = SimplifyTools.CollinearSimplify(tempP);
                    tempP.ForceCounterClockWise();
                    return new List<List<Vector2>> { tempP };
                }
            ...
            */
            ...
            if (triangulated.Count < 1)
            {
                ...
                throw new System.Exception("Error computing mass data - invalid polygon input");
            }

            List<List<Vector2>> polygonizedTriangles = PolygonizeTriangles(triangulated, maxPolys, tolerance);
            ...
            {
                polygonizedTriangles[i].CollinearSimplify(0);
            }
        ...
        ///<param name="tolerance">The tolerance</param>
        ///<returns></returns>
        public static List<List<Vector2>> PolygonizeTriangles(List<Triangle> triangulated, int maxPolys, float tolerance)
        {
            List<List<Vector2>> polys = new List<List<Vector2>>(50);
            ...
            while (notDone)
            {
                ...
                {
                    List<Vector2> poly = new List<Vector2>(3);
                    ...
                    for (int i = 0; i < 2 * triangulated.Count; ++i, ++index)
                    {
                        ...
                        List<Vector2> newP = AddTriangle(triangulated[index], poly);
                        if (newP == null)
                    ...
                    if (polyIndex < maxPolys)
                    {
                        ...
                        if (poly.Count >= 3)
                            polys.Add(new List<Vector2>(poly));
                        //else
        ...
        /// </summary>
        /// <returns></returns>
        public static List<Triangle> TriangulatePolygon(List<Vector2> vertices)
        {
            ...
            List<Vector2> pA, pB;
            List<Vector2> pin = new List<Vector2>(vertices);
        ...
        /// <returns></returns>
        private static bool ResolvePinchPoint(List<Vector2> pin, out List<Vector2> poutA, out List<Vector2> poutB)
        {
        ...
        private static List<Vector2> AddTriangle(Triangle t, List<Vector2> vertices)
        {
            ...
            List<Vector2> result = new List<Vector2>(vertices.Count + 1);
            for (int i = 0; i < vertices.Count; i++)
            {
