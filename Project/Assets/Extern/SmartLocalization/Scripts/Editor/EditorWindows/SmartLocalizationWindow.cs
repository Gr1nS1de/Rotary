...
#region Members
	...
	[SerializeField]
	bool generateAndroidPresence = false;
	[SerializeField]
	bool generateiOSPresence = false;
...
#region GUI Sections
	...
	{	
		...
		else if(label == "STOREPRESENCE")
		{
			DrawStorePresenceActions(position);
		}
		else
		{
	...
	public void DrawStorePresenceActions(Rect position)
	{
		float fullWindowWidth = position.width + 30;
		Rect newPosition = position;
		newPosition.width = fullWindowWidth;
		GUI.Label(newPosition, "Generate Store Presence - (BETA)", EditorStyles.boldLabel);

		newPosition.y += position.height * 0.15f;
		newPosition.width = 120;
		bool generateAndroidPresenceToggle = GUI.Toggle(newPosition, generateAndroidPresence, "Google Play");
		newPosition.x += 120;
		bool generateiOSPresenceToggle = GUI.Toggle(newPosition, generateiOSPresence, "App Store");

		if(generateAndroidPresenceToggle != generateAndroidPresence)
		{
			generateAndroidPresence = generateAndroidPresenceToggle;
			AndroidStorePresenceGenerator.GenerateStorePresence = generateAndroidPresence;
		}

		if(generateiOSPresenceToggle != generateiOSPresence)
		{
			generateiOSPresence = generateiOSPresenceToggle;
			IOSStorePresenceGenerator.GenerateStorePresence = generateiOSPresence;
		}
	}
